...
int get_valid_grid_neighbors(int neighbors_length, maze_tile_t *neighbors[], maze_tile_t *valid_neighbors[])
{
  ...
  for (int k = 0; k < neighbors_length; k++) {
    maze_tile_t *neighbor_tile = neighbors[k];
    if (neighbor_tile->type == GRID_TILE) {
      grid_maze_tile_t *neighbor = neighbor_tile->tile_data;
...
maze_tile_t* get_grid_perimeter_tile(int index, grid_maze_data_t *maze)
{
  ...
  if (cw_index < width_d) { // top
    ...
    i = cw_index - (width_d + height_d);
    j = height_d;
  } else { // left
    i = 0;
    j = cw_index - (width_d * 2 + height_d);
  }

  maze_tile_t *tile = &maze->tiles[j][i];
...
{
  if ((head->type == GRID_TILE) && (current->type == GRID_TILE)) {
